...
load("data/modelingset08c.Rdata") 
...
if (FALSE) { # do not re-tune
  ...
  #            write.csv(yhat.vl, paste("data/run", i, "_set", st, "_fold", k, ".csv", sep=""), row.names=FALSE)  
        ...
        save(list=c("logfile"), file="data/logfile10c.RData")
    }
...
write.csv(yhat+1, paste("models/xgboost_v10c_cv_5fold.csv", sep=""), row.names=FALSE)
...
write.csv(pred1,"models/xgboost_v10c_test_unrounded.csv",row.names=F, quote=FALSE)
# write.csv(pred2,"models/xgboost_v10c_test_rounded.csv",row.names=F, quote=FALSE)
