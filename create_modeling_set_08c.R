...
# features from Roman
X1 <- read.csv("data/X250_3.csv", header = FALSE)
X1.test <- read.csv("data/X250_test_3.csv",header = FALSE)
...
# alt_q alt_query_features_train_and_test_v01
f4 <- read.csv("data/alt_query_features_train_and_test_v01.csv", header = TRUE)
f4.test <- f4[(nrow(X2)+1):nrow(f4),]
f4<- f4[1:(nrow(X2)),]
...
save(list=c("X", "X.test", "y", "id.test"), file="data/modelingset08c.Rdata")
