...
X <- as.matrix(cbind(X2, X1, f1, f2, f3))
X.test <- as.matrix(cbind(X2.test, X1.test, f1.test, f2.test, f3.test))
y <- target - 1 # because xgboost expects levels to be named 0,1,...,n-1


# save modleing data for future use
save(list=c("X", "X.test","y", "id.test"), file="data/modelingset08.Rdata")
