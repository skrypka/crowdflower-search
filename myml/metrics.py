...
def normalized_gini(solution, submission):
    rez = gini(solution, submission)/gini(solution, solution)
    if type(rez) is np.ndarray:
        rez = rez[0]
    return rez

normalized_gini_scorer = metrics.make_scorer(normalized_gini, greater_is_better=True)
